...
(function(myGarden) {
  ...
  function levelUp() {
    ...
    if (myGarden.level % 10 == 0 || myGarden.level % 10 == 0) {
      addGarden();
      drawGarden(myGarden)
    }
    save();
  ...
  function plotclick() {
    ...
    if (plot.l > harvest) {
      ...
      addProgress(Math.ceil(myGarden.level / 50));
    ...
    addProgress(1);
    $(this).addClass(plotClass[plot.t]).append($('<div>').addClass('point-up').text('💦').on('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
      $(this).remove();
    }))
